...
const maxPan = new Vector3(2, 3, 3);
...
class OrbitControls extends Controls {
	...
	update( deltaTime = null ) {
        ...
        // If clamping changed the target, remove the extra pan that tried to push it past bounds
        // This prevents the camera from feeling like it "keeps trying" to pan through the wall.
        if (!this.target.equals(before)) {
        const correction = before.sub(this.target); // how much we had to pull it back

        if (this.enableDamping === true) {
            // remove the overshoot from the pan offset too
            this._panOffset.sub(correction.multiplyScalar(1 / this.dampingFactor));
        } else {
            this._panOffset.sub(correction);
        }
        }
